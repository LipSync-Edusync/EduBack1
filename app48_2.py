...
# app.config['MYSQL_HOST'] = '192.168.151.162'
...
def convert_mp4_to_wav(mp4_file_path):
    try:
        ...
        print(mp4_file_path)
        k = ffmpeg.input(mp4_file_path).output(wav_file_path, format='wav').run(overwrite_output=True)
        print(f"k = {k}")
...
#         # Forward request to second service on port 5001
#         try:
#             response = requests.get(f"http://192.168.0.128:5001/check-file", params={"filename": filename})
#             return jsonify({
#                 "local_exists": True,
...
#             }), 200
#         except Exception as e:
#             print(f"Error forwarding to 5001: {e}")
#             return jsonify({"local_exists": True, "remote_error": str(e)}), 500
